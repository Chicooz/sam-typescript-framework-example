...
import * as lambda from "../../../src/handlers/createContract";
...
const fakeusecase:UseCase<string> = mock(CreateContractUseCase);
...
describe("Testing createContract Handler", () => {
    ...
    it("should return success if useCase returns token", async () =>{
        when(fakeusecase.operate()).thenResolve("fakeContractID");
        const response = await lambda.handler(fakeEvent);
        expect(response).toEqual(new Success({ContractID:'fakeContractID'}))
    });
